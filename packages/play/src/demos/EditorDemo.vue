<script setup>
  ...
  // vjson 定义(reactive 对象, 编辑器就地修改): 覆盖静态/动态属性、事件(标识符与内联两种形式)、
  // v-if、v-for(keyField)、v-model / v-model:title、v-bind、v-show、默认/具名/作用域插槽、el-table
  // 其中 MyPanel 在 scriptSetup 中以 defineComponent 定义, 演示 type 取 "setup 上下文变量"
  const vjson = reactive({
    ...
    // 编辑器扩展字段: 组件级 CSS, 预览时动态注入, 随 vjson 一起导出
    style: `.vjson-demo .section {
  margin: 12px 0;
}
.vjson-demo button {
  margin-right: 8px;
